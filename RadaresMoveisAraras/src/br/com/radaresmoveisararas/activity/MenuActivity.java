...
public class MenuActivity extends ActionBarActivity{
	...
	public void selectItem(int pai, int filho) {
		mp.clear();
		RadarRadarLocal radar = (RadarRadarLocal) menuAdapter.getChild(pai, filho);
		...
		MarkerOptions marker = new MarkerOptions().position(start)
				.title(radar.radarLocal.nomeRua);
		marker.icon(BitmapDescriptorFactory
				.defaultMarker(BitmapDescriptorFactory.HUE_RED));
		marker.snippet(radar.radarLocal.descricao);
		mp.addMarker(marker).showInfoWindow();

		MarkerOptions marker2 = new MarkerOptions().position(end)
				.title(radar.radarLocal.nomeRua);
